module BlindfoldChessTest.BlindfoldChessMechanics.Notation.ParserTest
...
let ``Fen of initial position`` () =
    let act = "rnbqkbnr/pppppppp/8/8/8/8/PPPPPPPP/RNBQKBNR w KQkq - 0 1" |> textOfFen

    Assert.AreEqual(Position.init, act)

[<Test>]
let ``Fen of position after first half movement`` () =
    let act = "rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1" |> textOfFen

    Assert.AreEqual(PositionTest.positionAfterFirstHalfMovement, act)

[<Test>]
let ``Fen of position after second half movement`` () =
    let act =
        "rnbqkbnr/ppp1pppp/8/3p4/4P3/8/PPPP1PPP/RNBQKBNR w KQkq d6 0 2" |> textOfFen

    Assert.AreEqual(PositionTest.positionAfterSecondHalfMovement, act)

[<Test>]
let ``Fen of position after third half movement`` () =
    let act = "rnbqkbnr/ppp1pppp/8/3P4/8/8/PPPP1PPP/RNBQKBNR b KQkq - 0 2" |> textOfFen

    Assert.AreEqual(PositionTest.positionAfterThirdHalfMovement, act)

[<Test>]
let ``Fen of position after forth half move`` () =
    let act =
        "rnbqkb1r/ppp1pppp/5n2/3P4/8/8/PPPP1PPP/RNBQKBNR w KQkq - 1 3" |> textOfFen

    Assert.AreEqual(PositionTest.positionAfterForthHalfMove, act)

[<Test>]
let ``Fen of position after fifth half move`` () =
    let act =
        "rnbqkb1r/ppp1pppp/5n2/3P4/8/8/PPPPBPPP/RNBQK1NR b KQkq - 2 3" |> textOfFen

    Assert.AreEqual(PositionTest.positionAfterFifthHalfMove, act)

[<Test>]
let ``Fen of position after sixth half move`` () =
    let act =
        "rnbqkb1r/ppp2ppp/4pn2/3P4/8/8/PPPPBPPP/RNBQK1NR w KQkq - 0 4" |> textOfFen

    Assert.AreEqual(PositionTest.positionAfterSixthHalfMove, act)

[<Test>]
let ``Fen of position after seventh half move`` () =
    let act =
        "rnbqkb1r/ppp2ppp/4pn2/3P4/7P/8/PPPPBPP1/RNBQK1NR b KQkq h3 0 4" |> textOfFen

    Assert.AreEqual(PositionTest.positionAfterSeventhHalfMove, act)

[<Test>]
let ``Fen of position after eighth half move`` () =
    let act =
        "rnbqk2r/ppp2ppp/4pn2/3P4/7P/b7/PPPPBPP1/RNBQK1NR w KQkq - 1 5" |> textOfFen

    Assert.AreEqual(PositionTest.positionAfterEighthHalfMove, act)

[<Test>]
let ``Fen of position after ninth half move`` () =
    let act =
        "rnbqk2r/ppp2ppp/4pn2/3P4/7P/P7/P1PPBPP1/RNBQK1NR b KQkq - 0 5" |> textOfFen

    Assert.AreEqual(PositionTest.positionAfterNinthHalfMove, act)

[<Test>]
let ``Fen of position after tenth half move`` () =
    let act =
        "rnbq1rk1/ppp2ppp/4pn2/3P4/7P/P7/P1PPBPP1/RNBQK1NR w KQ - 1 6" |> textOfFen

    Assert.AreEqual(PositionTest.positionAfterTentHalfMove, act)

[<Test>]
let ``Fen of position after eleventh half move`` () =
    let act = "rnbq1rk1/ppp2ppp/4pn2/3P4/7P/P7/P1PPBPP1/RNBQ1KNR b - - 2 6" |> textOfFen

    Assert.AreEqual(PositionTest.positionAfterEleventhHalfMove, act)

[<Test>]
let ``Fen of position after twelfth half move`` () =
    let act =
        "rnbq1rk1/pp3ppp/4pn2/2pP4/7P/P7/P1PPBPP1/RNBQ1KNR w - c6 0 7" |> textOfFen

    Assert.AreEqual(PositionTest.positionAfterTwelfthHalfMove, act)

[<Test>]
let ``Fen of position after thirteenth half move`` () =
    let act = "rnbq1rk1/pp3ppp/2P1pn2/8/7P/P7/P1PPBPP1/RNBQ1KNR b - - 0 7" |> textOfFen

    Assert.AreEqual(PositionTest.positionAfterThirteenthHalfMove, act)
...
let ``Text of game without meta tags`` () =
    ...
    let act = game.Moves |> Array.length

    Assert.AreEqual(99, act)
...
Qh8+ Kg2 68.Kd3 Kg1 69.Ke2 g2 70.Qd4+ Kh1 71.Qh4+ Kg1 1-0"""
    ...
    let act = largeGame.Moves |> Array.length

    Assert.AreEqual(142, act)

[<Test>]
let ``Json of example game`` () =
    let act = ResourcesAsCode.exampleGameJson |> jsonOfGame

    Assert.AreEqual(EmitterTest.exampleGame, act)
